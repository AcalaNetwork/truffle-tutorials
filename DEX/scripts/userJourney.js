...
const { ACA, AUSD, DEX, DOT } = require('@acala-network/contracts/utils/MandalaTokens');
...
module.exports = async function (callback) {
  try {
    ...
    console.log(
      'Expected target when using path ACA -> AUSD: %s AUSD',
      formatUnits(expectedTarget1.toString(), ausdDecimals)
    );
    console.log(
      'Expected target when using path ACA -> AUSD -> DOT: %s DOT',
      formatUnits(expectedTarget2.toString(), dotDecimals)
    );
    ...
    console.log(
      ...
      formatUnits(targetAusd.toString(), ausdDecimals),
      formatUnits(targetAusd.add(halfwayAusdBalance).toString(), ausdDecimals),
      formatUnits(expectedSupply1.toString(), acaDecimals)
    );
    console.log(
      'Expected supply for getting %s DOT in order to reach a total of %s DOT is %s ACA.',
      formatUnits(targetDot.toString(), dotDecimals),
      formatUnits(targetDot.add(halfwayDotBalance).toString(), dotDecimals),
      formatUnits(expectedSupply2.toString(), acaDecimals)
    );
    ...
    await instance.swapWithExactTarget(
      path1,
      targetAusd,
      expectedSupply1.add(web3.utils.toBN(parseUnits('1', ausdDecimals)))
    );
    await instance.swapWithExactTarget(
      path2,
      targetDot,
      expectedSupply2.add(web3.utils.toBN(parseUnits('1', dotDecimals)))
    );
    ...
    console.log(
      ...
      formatUnits(finalAusdBalance.sub(halfwayAusdBalance).toString(), ausdDecimals),
      formatUnits(targetAusd.toString(), ausdDecimals)
    );
    console.log(
      'DOT balance has increased by %s DOT, while the expected increase was %s DOT.',
      formatUnits(finalDotBalance.sub(halfwayDotBalance).toString(), dotDecimals),
      formatUnits(targetDot.toString(), dotDecimals)
    );

    console.log(
      'Expected decrease of ACA balance was %s ACA, while the actual decrease was %s ACA.',
      formatUnits(expectedSupply1.add(expectedSupply2).toString(), acaDecimals),
      formatUnits(halfwayAcaBalance.sub(finalAcaBalance).toString(), acaDecimals)
    );
