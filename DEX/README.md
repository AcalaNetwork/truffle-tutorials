...
`DEX` from `@acala-network/contracts` dependency. Your skeleton smart contract should look like
this:
...
Import of the `DEX` from `@acala-network/contracts` is done between the `pragma` definition and
...
As we now have access to `DEX.sol` from `@acala-network/contracts`, we can set the inheritance of
our `PrecompiledDEX` contract:
...
Tests for this tutorial will validate the expected values returned and expected behaviour of DEX
predeployed smart contract. The test file in our case is called `DEX.js`. Within it we import the
...
To prepare for the testing, we have to define four global variables, `instance`, `AUSDinstance`,
`ACAinstance` and `deployer`. The `instance` will store the predeployed DEX smart contract instance.
`AUSDinstance` and `ACAinstance` will store the values of the token predeployed stmart constracts.
he `deployer` will store the account that we will be using in our tests. Let's assign them values in
the `beforeEach` action:
...
`@acala-network/contracts`, `formatUnits` and `parseUnits` from `ethers` and precompiled `DEX` and
...
beginning of the script, we assign an address value to `deployer`, get the its initial native
